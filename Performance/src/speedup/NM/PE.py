...
class PE():
    def __init__(self,input_param,tech,cell):
        ...
        if self.conf.weightmapping == "Sign" and self.conf.signmapping == "NPsplit":
            ...
            ## since we always consider the shift is positive and the sign is encoded as digit add or minus, the negative array will not include dummy column => zero dummy to make the hardware consistant
            #self.Array_n.WeDummyCol = 0
    ...
    def Configure(self):
        ...
        #self.bufferInputCore.Configure((self.bufferCoreRow*self.bufferCoreCol) , self.conf.clkFreq)
        #self.bufferInputcoreNum = np.ceil(self.conf.numRowSubArray *  self.conf.numBitInput * self.NumRows / (self.bufferCoreRow * self.bufferCoreCol))
    ...
    def CalculatePerformance(self,average_activityRowRead,input,inputshift,average_Condutance,weight,weight_n,average_dummy_conductance,speedup):
        ...
        #different image will give different input and thus different trace, could loop accross different image,
        # the number of image is controlled by the conf param, should be no bigger than data batchsize
        opoutputprecision = 0
        for batch_index in range(self.conf.batchsize):
            ...
            #first calculate the input shift and save it for further calculation
            if self.conf.inputshift != 0:
                # inputshift is shared by all the input vector and weight are fixed for the inference, thus only need to calculate once and used for all the input vector
                input_vector = np.ones(self.conf.numRowSubArray * weight_row) * np.abs(inputshift)
                ...
                self.readLatency += readLatencypervector
                self.readDynamicEnergy += readDynamicEnergypervector
    ...
    def traced_Array_performance(self,average_activityRowRead,input_vector,average_Condutance,weight,weight_n,average_dummy_conductance,weight_col,weight_row):
        ...
        input_vector = input_bin_list[0][0:self.conf.numRowSubArray]
        weight_subarray = weight[0:self.conf.numRowSubArray, 0:int(self.conf.numColSubArray / self.DigitPerWeight)]
        ...
        # average_activityRowRead_Subarray = 0.1
        ...
        if self.Array.WeDummyCol:
            #subarray(ADC) calculate latency for each res value and return the maximum
            #subarray(ADC) calculate power for each res value and return the accumulated value
            #dummy considered as additionall col
        ...
        self.bitop += weight_col* weight_row * self.conf.numBitInput * 2 * self.conf.numRowSubArray * self.conf.numColSubArray
    ...
    def conf_neurosim_array(self,input_param,tech,cell,conf):
        ...
        # i need to move it to upper layer
        if conf.wireWidth == -1:
