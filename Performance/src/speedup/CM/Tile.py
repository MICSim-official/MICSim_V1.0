import neurosim
import numpy as np
from Performance.src.speedup.CM.PE import PE
...
class Tile():
    ...
    def CalculatePerformance(self,average_activityRowRead,input,inputshift,average_Condutance, weight, weight_n, average_dummy_conductance,speedup):
        ...
        self.readLatency = self.PE.readLatency / DupPEnum
        self.SubArrayreadLatency = self.PE.SubArrayreadLatency / DupPEnum
        self.BufferreadLatency = self.PE.BufferreadLatency /DupPEnum
        self.ICreadLatency = self.PE.ICreadLatency / DupPEnum
        self.DigitreadLatency = self.PE.DigitreadLatency/ DupPEnum
        # add for adc
        self.ADCreadLatency = self.PE.ADCreadLatency/ DupPEnum
        
        
        self.readDynamicEnergy +=  NumPE_need * self.PE.readDynamicEnergy
        self.SubArrayreadDynamicEnergy += NumPE_need * self.PE.SubArrayreadDynamicEnergy
        self.BufferreadDynamicEnergy += NumPE_need * self.PE.BufferreadDynamicEnergy
        self.ICreadDynamicEnergy += NumPE_need * self.PE.ICreadDynamicEnergy
        self.DigitreadDynamicEnergy += NumPE_need * self.PE.DigitreadDynamicEnergy
        
        self.ADCreadDynamicEnergy +=NumPE_need*self.PE.ADCreadDynamicEnergy
        ...
        self.writeDynamicEnergy += NumPE_need * self.PE.writeDynamicEnergy
        self.SubArraywriteDynamicEnergy += NumPE_need * self.PE.SubArraywriteDynamicEnergy
        self.BufferwriteDynamicEnergy += NumPE_need * self.PE.BufferwriteDynamicEnergy
        self.ICwriteDynamicEnergy += NumPE_need * self.PE.ICwriteDynamicEnergy
