...
class PE():
    ...
    def CalculatePerformance(self,average_activityRowRead, input,inputshift,average_Condutance,weight,weight_n,average_dummy_conductance,speedup):
        ...
        if weight_row > 1:
            ...
            # self.AdderTree.CalculatePower(self.conf.batchsize*num_vector*(weight_col*weight.shape[1]), weight_row)
    ...
    def traced_Array_performance(self,average_activityRowRead,input_vector,average_Condutance,weight,weight_n,average_dummy_conductance,weight_col,weight_row):
        ...
        for array_col_index in range(weight_col):
            for array_row_index in range(weight_row):
                ...
                cinstart = array_row_index   * self.SubarrayRows
                cinend = (array_row_index+1) * self.SubarrayRows
                coutstart = array_col_index * int(self.SubarrayCols / self.DigitPerWeight)
                coutend =(array_col_index+1)* int(self.SubarrayCols / self.DigitPerWeight)
                ...
                if self.SubArraywriteLatency < self.Array.writeLatency:
                    self.SubArraywriteLatency = self.Array.writeLatency
                self.SubArraywriteDynamicEnergy += self.Array.writeDynamicEnergy


                #count the bit op 1bit weight with 1bit input
                self.bitop += self.conf.numBitInput * 2 * self.SubarrayRows * self.SubarrayCols 
                if weight_n == True:
                    SubArrayreadDynamicEnergy += self.conf.numBitInput * self.Array.readDynamicEnergy
                    ADCreadDynamicEnergy += self.conf.numBitInput * self.Array.readDynamicEnergyADC
                    self.bitop += self.conf.numBitInput * 2 * self.SubarrayRows * self.SubarrayCols 
