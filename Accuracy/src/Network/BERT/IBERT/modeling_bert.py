...
from Accuracy.src.Layers.QLayer.Transformer.QEmbedding import QEmbedding
from Accuracy.src.Layers.QLayer.Transformer.IntSoftmax import IntSoftmax
from Accuracy.src.Layers.QLayer.Transformer.IntGELU import IntGELU
from Accuracy.src.Layers.QLayer.Transformer.IntLayerNorm import IntLayerNorm
from Accuracy.src.Modules.Transformer.Quantizer.IBERTQuantizer import QuantAct
from Accuracy.src.Layers.QLayer.Transformer.QLinear import QLinear
from Accuracy.src.Layers.QLayer.Transformer.Matmul import Matmul as CIM_Matmul
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = QEmbedding(
            config.vocab_size,
            config.hidden_size,
        ...
        # )
        self.token_type_embeddings = QEmbedding(
            config.type_vocab_size, config.hidden_size
        )
        self.position_embeddings = QEmbedding(
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        # Requantization (32bit -> 8bit) for Q, K, V activationt
        self.query_activation = QuantAct(self.act_bit, quant_mode=self.quant_mode,need_Oint=True)
        self.key_activation = QuantAct(self.act_bit, quant_mode=self.quant_mode,need_Oint=True)
        self.value_activation = QuantAct(self.act_bit, quant_mode=self.quant_mode,need_Oint=True)
        self.output_activation = QuantAct(self.act_bit, quant_mode=self.quant_mode)
        
        self.softmax = IntSoftmax(self.act_bit, quant_mode=self.quant_mode, need_Oint=True,force_dequant=force_dequant)
    ...
    def forward(
        ...
        self.query.dumpname = bertlayer_name + '_selfattention_weight_query_layer'
        self.key.dumpname = bertlayer_name + '_selfattention_weight_key_layer'
        self.value.dumpname = bertlayer_name + '_selfattention_weight_value_layer'
        ...
        MatmulKQ_name = bertlayer_name + '_selfattention_MatmulKQ'
        attention_scores = CIM_Matmul(query_layer,query_layer_scaling_factor, key_layer.transpose(-1, -2),key_layer_scaling_factor,type='KQ',dumpname=MatmulKQ_name)
        ...
        # attention_probs_int = attention_probs / attention_probs_scaling_factor 
        attention_probs_scaling_factor_tensor = torch.tensor(attention_probs_scaling_factor)
        # input_is_integer = torch.remainder(attention_probs_int, 1) == 0
        # input_exists_false = not torch.all(input_is_integer)
        n = 2 ** (self.act_bit - 1) - 1
        attention_probs = torch.clamp(attention_probs, -n, n - 1)  
        if torch.cuda.is_available():
            attention_probs_scaling_factor_tensor=attention_probs_scaling_factor_tensor.cuda() 
        
        MatmulPV_name = bertlayer_name + '_selfattention_MatmulPV'
        context_layer = CIM_Matmul(attention_probs,attention_probs_scaling_factor_tensor,value_layer, value_layer_scaling_factor,type='PV',dumpname=MatmulPV_name)
        # context_layer [32, 12, 128, 64]
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.output_activation = QuantAct(self.act_bit, quant_mode=self.quant_mode)
        # self.dense = QLinear(config.hidden_size, config.hidden_size,quantize_weight=True, quantize_input=True, quantize_error=False)
        # self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, hidden_states, hidden_states_scaling_factor, input_tensor, input_tensor_scaling_factor):
        ...
        self.dense.dumpname = self.dumpname
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self.output.dumpname = bertlayer_name + '_selfoutput_layer'
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        # self.dense = QLinear(config.hidden_size, config.intermediate_size,quantize_weight=True, quantize_input=True, quantize_error=False)
        if config.hidden_act != "gelu":
            raise ValueError("I-BERT only supports 'gelu' for `config.hidden_act`")
        self.intermediate_act_fn = IntGELU(quant_mode=self.quant_mode, force_dequant=force_dequant)
        self.output_activation = QuantAct(self.act_bit, quant_mode=self.quant_mode)

    def forward(self, hidden_states, hidden_states_scaling_factor,bertlayer_name=None):
        self.dense.dumpname=self.dumpname
        hidden_states, hidden_states_scaling_factor = self.dense(hidden_states, hidden_states_scaling_factor)
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_scaling_factor, input_tensor, input_tensor_scaling_factor,bertlayer_name=None):
        self.dense.dumpname=self.dumpname
        hidden_states, hidden_states_scaling_factor = self.dense(hidden_states, hidden_states_scaling_factor)
        hidden_states = self.dropout(hidden_states)
        hidden_states, hidden_states_scaling_factor = self.ln_input_act(
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs, self_attention_outputs_scaling_factor = self.attention(
            ...
            bertlayer_name=self.dumpname
        
        )
        
        self.intermediate.dumpname = self.dumpname +'_intermediate_layer'
        self.output.dumpname = self.dumpname + '_output_layer'
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # assign name for each BertLayer
        for i, layer in enumerate(self.layer):
            layer.dumpname = f'BertLayer_{i}'
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (QEmbedding, nn.Embedding)):
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
